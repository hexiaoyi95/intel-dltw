...
class CaffeBackend():
    ...
    def prepare_infer(self, config):
        if  config.engine or config.engine == "default":
        ...
        input_list = utils.io.get_input_list(config.input_path, config.batch_size + 1)

        inputs = self.image_preprocess(input_list)

        self.set_input(inputs)


    def image_preprocess(img_list, **kwargs):
    """
    ...
    """
        ...
        #transform
        inputs = [transformer.preprocess(net.inputs[0], img)
              for img in inputs]
